...
insta_line_a = insta %>%   
  ...
  ungroup() %>%
  plot_ly(x = ~days_since_prior_order, y = ~n, name = "Lunch meat", type = "scatter", 
          mode = "lines+markers", marker = list(color = "c3d6c8"), 
          line = list(color = "c3d6c8", width = 4)) %>% 
...
insta_line_b = insta %>%   
  ...
  count(days_since_prior_order)  %>%
  ungroup() %>%
  plot_ly(x = ~days_since_prior_order, y = ~n, name = 'Prepared meals', type = "scatter", 
          mode = "lines+markers", marker = list(color = "gold"), line = list(color = "gold", width = 4))  %>% 
  layout(plot_bgcolor = "fffefa",
         xaxis = list(title = "Days since prior order"), 
         yaxis = list(title = "Number of orders")) 
...
insta_line_c = insta %>%   
  ...
  count(days_since_prior_order)  %>%
  ungroup() %>%
  plot_ly(x = ~days_since_prior_order, y = ~n, name = 'Fresh dips and tapenades', type = "scatter", 
